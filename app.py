...
# Version / build labels (configurable, real values shown in the panel)
...
# Real service running state. When stopped, every tunnel is closed and new
# WS connections are refused — exactly like stopping the core in a real panel.
...
def get_domain() -> str:
    return os.environ.get("RENDER_EXTERNAL_URL", os.environ.get("SPACE_HOST", os.environ.get("RAILWAY_PUBLIC_DOMAIN", "localhost"))).replace("https://", "").replace("http://", "")

def generate_uuid(seed: str | None = None) -> str:
    # Real, standard RFC-4122 UUID. This value is the ONLY credential for a
    # connection: the WS tunnel authorises strictly by /ws/{uuid}. Change the
    # UUID in the client config and it no longer matches a key in LINKS, so the
    # tunnel is rejected. Nothing here is cosmetic.
    if seed is None:
...
body{font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#0a1222;color:#fff;min-height:100vh;overflow:hidden}
.login-app{height:100vh;display:flex;flex-direction:column}
.under{flex:1;background:#0a1222;position:relative;z-index:0;min-height:0}
.waves-header{position:fixed;width:100%;text-align:center;background:#0a1222;z-index:-1;top:0}
.waves-inner-header{height:50vh;width:100%}
.waves{position:relative;width:100%;height:15vh;margin-bottom:-8px;min-height:100px;max-height:150px}
.parallax>use{animation:move-forever 25s cubic-bezier(.55,.5,.45,.5) infinite}
.parallax>use:nth-child(1){animation-delay:-2s;animation-duration:4s;opacity:.2}
.parallax>use:nth-child(2){animation-delay:-3s;animation-duration:7s;opacity:.4}
.parallax>use:nth-child(3){animation-delay:-4s;animation-duration:10s;opacity:.6}
.parallax>use:nth-child(4){animation-delay:-5s;animation-duration:13s}
@keyframes move-forever{0%{transform:translate3d(-90px,0,0)}100%{transform:translate3d(85px,0,0)}}
.h-100{height:100%}
.overflow-y-auto{overflow-y:auto}
.ant-row-flex{display:flex;flex-wrap:wrap}
.ant-row-flex-center{justify-content:center;align-items:center}
.ant-row-flex-middle{align-items:center}
.my-3rem{margin:3rem 0}
.ant-col{position:relative;min-height:1px;padding:0}
.ant-col-xs-22{width:91.6667%}
.ant-col-sm-12{width:50%}
.ant-col-md-10{width:83.3333%}
.ant-col-lg-8{width:66.6667%}
.ant-col-xl-6{width:50%}
.ant-col-xxl-5{width:41.6667%}
#login{background:#151f31;border-radius:2rem;padding:4rem 3rem;transition:.3s;animation:charge .5s ease both;max-width:420px;margin:auto}
#login:hover{box-shadow:0 2px 8px rgba(0,0,0,.09)}
@keyframes charge{0%{transform:translateY(5rem);opacity:0}100%{transform:translateY(0);opacity:1}}
.setting-section{position:absolute;top:0;right:0;padding:22px}
.ant-btn-circle{border-radius:50%;width:32px;height:32px;padding:0;border:1px solid rgba(255,255,255,.1);background:transparent;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center}
.ant-btn-circle:hover{background:rgba(255,255,255,.05)}
.title{font-size:2rem;margin-bottom:2rem;text-align:center}
...
.ant-form-horizontal{width:100%}
.ant-space{display:flex;flex-direction:column;gap:16px;width:100%}
.ant-form-item{margin:0}
.ant-form-item-control-wrapper{width:100%}
.ant-form-item-control{position:relative}
.ant-input-affix-wrapper{display:flex;align-items:center;background:#1a1a1a;border:1px solid #2a2a2a;border-radius:30px;padding:0 16px;height:50px;transition:border-color .3s}
.ant-input-affix-wrapper:focus-within{border-color:#008771;box-shadow:0 0 0 2px rgba(0,135,113,.2)}
.ant-input-prefix{display:flex;align-items:center;margin-right:10px;color:#666}
.ant-input{flex:1;background:transparent;border:none;outline:none;color:#fff;font-size:14px;height:100%;padding:0}
.ant-input::placeholder{color:#555}
.ant-input-suffix{display:flex;align-items:center;color:#666;cursor:pointer;padding-left:8px}
.ant-input-suffix:hover{color:#fff}
.centered{display:flex;text-align:center;align-items:center;justify-content:center;width:100%}
.wave-btn-bg{position:relative;border-radius:25px;width:100%;transition:.3s cubic-bezier(.645,.045,.355,1)}
.wave-btn-bg-cl{border-radius:3em;background-image:linear-gradient(rgba(255,255,255,0),rgba(255,255,255,0)),radial-gradient(circle at left top,#005565,#008771,#005565);background-size:300%;background-position:0 0;width:100%;z-index:1}
.wave-btn-bg-cl:hover{animation:wave-btn-tara 4s ease infinite}
.wave-btn-bg-cl::before{position:absolute;content:'';inset:-5px;z-index:-1;background:inherit;border-radius:4em;opacity:0;transition:.5s}
.wave-btn-bg-cl:hover::before{opacity:1;filter:blur(20px);animation:wave-btn-tara 8s linear infinite}
@keyframes wave-btn-tara{100%{background-position:300% center}}
.h-50px{height:50px}
.mt-1rem{margin-top:1rem}
.ant-btn-primary-login{font-size:14px;color:#fff;background:transparent;border:none;border-radius:2rem;height:46px;width:100%;cursor:pointer;position:relative;text-align:center;padding:0 15px}
...
@media(max-width:768px){.waves{height:40px;min-height:40px}#login{padding:2rem 1.5rem;margin:1rem}}
</style>
</head>
<body>
<section class="login-app">
  <main class="under">
    <div class="waves-header">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 24 150 28" preserveAspectRatio="none" shape-rendering="auto" class="waves">
        <defs><path id="gentle-wave" d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z"></path></defs>
        <g class="parallax">
          <use href="#gentle-wave" x="48" y="0" fill="rgba(0,135,113,0.08)"></use>
          <use href="#gentle-wave" x="48" y="3" fill="rgba(0,135,113,0.08)"></use>
          <use href="#gentle-wave" x="48" y="5" fill="rgba(0,135,113,0.08)"></use>
          <use href="#gentle-wave" x="48" y="7" fill="#0a1a2a"></use>
        </g>
      </svg>
    </div>
    <div class="h-100 overflow-y-auto ant-row-flex ant-row-flex-center ant-row-flex-middle">
      <div class="my-3rem ant-col ant-col-xs-22 ant-col-sm-12 ant-col-md-10 ant-col-lg-8 ant-col-xl-6 ant-col-xxl-5" id="login">
            ...
            <form class="ant-form ant-form-horizontal" id="login-form">
              <div class="ant-space ant-space-vertical">
                ...
                <div class="ant-space-item">
                  ...
                  <div class="centered ant-row">
                    <div class="wave-btn-bg wave-btn-bg-cl h-50px mt-1rem" style="display:inline-block">
...
function toggleTheme() {
  darkTheme = !darkTheme;
  document.querySelector('.under').style.background = darkTheme ? '#0a1222' : '#f0f2f5';
  document.querySelector('.waves-header').style.background = darkTheme ? '#0a1222' : '#c7ebe2';
  document.getElementById('login').style.background = darkTheme ? '#151f31' : '#fff';
  document.getElementById('login').style.color = darkTheme ? '#fff' : '#000';
}
